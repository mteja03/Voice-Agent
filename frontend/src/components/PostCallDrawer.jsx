...
export default function PostCallDrawer({ isOpen, summary, leadName, onClose, onSave, onSaveAndNext }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md h-full bg-gray-900 border-l border-gray-700/50 flex flex-col shadow-2xl overflow-y-auto">
        <div className="px-6 py-5 border-b border-gray-800">
          <p className="text-xs text-gray-500 uppercase tracking-wider">Post-call review</p>
          <h3 className="text-base text-white font-semibold mt-1">{leadName || 'Selected lead'}</h3>
        </div>

        <div className="flex-1 px-6 py-5 space-y-4">
          <Field label="Outcome">
            <select
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-slate-500/40 focus:border-slate-500"
              value={form.outcome}
          ...
          <Field label="Summary note">
            <textarea
              className="w-full min-h-24 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-slate-500/40 focus:border-slate-500"
        ...
        <div className="px-6 py-4 border-t border-gray-800 flex items-center gap-2">
